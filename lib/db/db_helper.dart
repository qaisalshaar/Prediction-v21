...
// Query by Table Name -- Read Data
  ...
  static Future<List<Collection>> getcolliction() async {
    ...
    List<Map<String, dynamic>> maps = await _db!
       ...
       $_userHealth.dietfood, $_userHealth.medicineadvise
    ...
    return List.generate(maps.length, (i) {
      return Collection(
        ...
        dietfood: maps[i]['dietfood'],
        medicineadvise: maps[i]['medicineadvise'],
        // Same for the other properties
      );
    });
  }
...
// User Login
  ...
  static Future updateDoctorAdvice(
      ...
      String dietfood = '',
      String medicineadvise = ''}) async {
    // Update some record
    await _db!.rawQuery('''
      UPDATE $_userHealth SET 'medicineadvise' = '$medicineadvise' ,'dietfood' = '$dietfood'
      WHERE email='$email';
      ''');
  }
